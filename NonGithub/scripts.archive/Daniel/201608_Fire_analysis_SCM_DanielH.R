...
# for analysis of population structure of Carnegiea gigantea
...
burned.stats <- read.csv (paste0 (dat.dir, 'Burned.stats.csv'), as.is = T)
#--Unburned site data
...
otu.all$otu.count <- 1
otu.all %>%
  select (OTU.95, Sampling_number, otu.count, tree_number, elevation, group) %>%
...
tree <- burned.stats$tree.number
for (i in tree) {
  burned.stats [burned.stats$tree.number == i, 'ab.dry'] <-
    burned.stats [burned.stats$tree.number == i, 'total.colonized.root'] /
    burned.stats [burned.stats$tree.number == i, 'dry.root.mass']
  burned.stats [burned.stats$tree.number == i, 'ab.total.rt.tip'] <-
    burned.stats [burned.stats$tree.number == i, 'total.colonized.root'] /
    burned.stats [burned.stats$tree.number == i, 'total.root.tips']
}
...
ab.out <- data.frame (site = c(rep('Burned', 10), rep ('Unburned', 11)), abundance = NA)
...
# colors for points
color.vec <- data.frame (color = rep (NA, 21), p.group = otu.data$group)
color.vec <- sapply (color.vec$p.group, function (x) 
  if (x == 'Burned') {color.vec = 'black'} else {color.vec = 'grey'})
...
ordihull(horn.abund, groups = otu.data$group)

#--ANOSIM: Morisita
horn.anosim <- anosim (comm.matrix, grouping = otu.data$group, distance = "horn")
...
#--Check variance of distance matrix; p < 0.05 means that data is not normally distributed
#--can't use this data for anosim; must use non=parametric analyses (e.g. permanova)
anova(betadisper(comm.dist.jaccard, otu.data$group))

#--NMDS analysis
jaccard.abund <- metaMDS(comm.dist.horn, dist = "bray", permutations = 999)
...
#legend("topright", legend = levels(otu.env.data$group), bty = "n",
#       col = color.vec, pch = 21, pt.bg = color.vec, cex = 2)
ordihull(jaccard.abund, groups = otu.data$group)
