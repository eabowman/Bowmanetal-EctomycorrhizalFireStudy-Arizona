...
FU.matrix <- FU.matrix[!FU.matrix$Tree %in% c('NF19'),]

#--isolate site X species matrix only without metadata
comm.matrix <- FU.matrix[12:length(FU.matrix)]
...
color.vec[color.vec$p.group == 'santa.catalina', 'shape'] <- 1
color.vec[color.vec$p.group == 'pinaleno', 'shape'] <- 0
...
#--isolate points using rows isolated above
points(jaccard.otu$points[pinaleno.unburn,1:2], cex = 3,
       pch = color.vec[color.vec$p.group == 'pinaleno',
                       'shape'],
       col = 'black',
       bg = 'black')
points(jaccard.otu$points[santa.catalina.unburn,1:2], cex = 3,
       pch = color.vec[color.vec$p.group == 'santa.catalina' ,
                       'shape'],
       col = 'black',
       bg = 'black')

legend("topright", legend = c('Santa Catalina Mts.',
                                 'Pinaleno Mts.'), bty = "n",
       col = c('black', 'black'),
       pch = c(1,0),
       pt.bg =  c('black', 'black'), cex = 2)
...
#<< ANOSIM >>-----------------------------------------------------------------------------
jaccard.anosim <- anosim(comm.matrix, grouping = jaccard.matrix$Range,
                         distance = "jaccard")
jaccard.anosim
...
legend("bottomright", legend = c('Santa Catalina Mts.',
                                 'Pinaleno Mts.'), bty = "n",
       col = c('black', 'black'),
       pch = c(1,0),
       pt.bg =  c('black', 'black'), cex = 2)
...
#<< ANOSIM >>-----------------------------------------------------------------------------
morisita.anosim <- anosim(comm.matrix, grouping = morisita.matrix$Range,
                         distance = "horn")
morisita.anosim

#--Add results to data frame
anosim.res[which(anosim.res$anosim.res =="morisita.fu"), "r"] <- morisita.anosim$statistic
anosim.res[which(anosim.res$anosim.res =="morisita.fu"), "p"] <- morisita.anosim$signif
...
#--isolate site X species matrix only without metadata
comm.matrix <- FA.matrix[12:length(FA.matrix)]
...
#--Add results to data frame
anosim.res[which(anosim.res$anosim.res =="jaccard.fa"), "r"] <- jaccard.anosim$statistic
anosim.res[which(anosim.res$anosim.res =="jaccard.fa"), "p"] <- jaccard.anosim$signif
...
#<< PERMANOVA >>-----------------------------------------------------------------------------
morisita.anosim <- adonis(comm.dist.morisita ~ Range, 
                          data = morisita.matrix,
                          permutations = 1000)
morisita.anosim

#--Burn f.model, r2, p-value
anosim.res[which(anosim.res$anosim.res == "morisita.fa"), "F.model.burn"] <-
  morisita.anosim$aov.tab$F.Model[1]
anosim.res[which(anosim.res$anosim.res == "morisita.fa"), "r2.burn"] <-
  morisita.anosim$aov.tab$R2[1]
anosim.res[which(anosim.res$anosim.res == "morisita.fa"), "p.burn"] <-
  morisita.anosim$aov.tab$`Pr(>F)`[1]
#--export anosim table
write.csv(anosim.res, paste0(res.dir, "ANOSIM_SequenceBased.csv"),
          row.names = F)

#========================================================================================#
# Jaccard based dissimilarity index: mean pairwise similarity----
#========================================================================================#
...
# add row names
rownames(comm.matrix) <- jaccard.matrix$Tree

#--distance matrix using jaccard index
comm.dist.jaccard <- vegdist(comm.matrix, method = 'jaccard', binary = F, na.rm = T)
comm.dist.matrix <- as.data.frame(as.matrix(comm.dist.jaccard))
comm.dist.matrix$X <- rownames(comm.dist.matrix)
sim.jaccard <- gather(comm.dist.matrix, 'tree2','dissimilarity', -X)
names(sim.jaccard)[1] <- 'tree1'
sim.jaccard$range.1 <- NA
sim.jaccard$fire.history.1 <- NA
sim.jaccard$range.2 <- NA
sim.jaccard$fire.history.2 <- NA
for(i in unique(sim.jaccard$tree1)){
  for(t in unique(sim.jaccard$tree2)){
    sim.jaccard[sim.jaccard$tree1 == i, 'range.1'] <- 
      jaccard.matrix[jaccard.matrix$Tree == i, 'Range'] 
    sim.jaccard[sim.jaccard$tree1 == i, 'fire.history.1'] <- 
      jaccard.matrix[jaccard.matrix$Tree == i, 'Burn_status'] 
    sim.jaccard[sim.jaccard$tree2 == t, 'range.2'] <- 
      jaccard.matrix[jaccard.matrix$Tree == t, 'Range'] 
    sim.jaccard[sim.jaccard$tree2 == t, 'fire.history.2'] <- 
      jaccard.matrix[jaccard.matrix$Tree == t, 'Burn_status'] 
  }
}

#add comparison column
sim.jaccard[sim.jaccard$range.1 == sim.jaccard$range.2, 'range.comp'] <- 'within'
sim.jaccard[!sim.jaccard$range.1 == sim.jaccard$range.2, 'range.comp'] <- 'between'
sim.jaccard[sim.jaccard$fire.history.1 == sim.jaccard$fire.history.2, 'fire.comp'] <- 'within'
sim.jaccard[!sim.jaccard$fire.history.1 == sim.jaccard$fire.history.2, 'fire.comp'] <- 'between'

#Similarity column
sim.jaccard$similarity <- 1-sim.jaccard$dissimilarity
sim.jaccard$log.similarity <- log(sim.jaccard$similarity)

#Remove self comparisons
sim.jaccard <- sim.jaccard[!sim.jaccard$similarity == 1, ]

ggplot(data = sim.jaccard, mapping = aes(x = fire.comp,
                                         y = log.similarity)) +
  geom_boxplot()  +
  facet_grid(. ~ fire.history.1)
